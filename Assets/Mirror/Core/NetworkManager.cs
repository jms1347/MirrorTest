...
namespace Mirror
{
    ...
    {
        /// <summary>장면을 변경할 때 NetworkManager를 활성 상태로 유지할 수 있습니다.</summary>
        // 게임에 프로세스 수명 동안 존재하는 단일 NetworkManager가 있는 경우 설정해야 합니다.
        // 각 장면에 NetworkManager가 있는 경우 이를 설정하면 안 됩니다.</para>[Header("Configuration")]
        ...
        /// <summary>멀티플레이어 게임은 네트워크가 시간 초과되지 않도록 항상 백그라운드에서 실행되어야 합니다.</summary>
        ...
        /// <summary>빌드 설정에서 'Server Build'를 체크하면 서버가 자동으로 시작되어야 할까요</summary>
        ...
        /// <summary>초당 서버 업데이트 빈도입니다. 
        /// Counter-Strike와 같이 빠르게 진행되는 게임에서는 대기 시간을 최소화하기 위해 약 60Hz를 사용합니다. 
        /// WoW와 같은 게임에서는 계산을 최소화하기 위해 약 30Hz를 사용합니다. EVE와 같이 속도가 느린 게임에는 약 1~10Hz를 사용하세요.</summary>[Tooltip("Server & Client send rate per second. Use 60-100Hz for fast paced games like Counter-Strike to minimize latency. Use around 30Hz for games like WoW to minimize computations. Use around 1-10Hz for slow paced games like EVE.")]
        [FormerlySerializedAs("serverTickRate")]
        public int sendRate = 60;

        // 2023년 11월 25일에 지원 중단됨
        // SerializeField 및 HideInInspector를 사용하여 자체 수정
        // headlessStartMode로 대체됩니다. 이는 나중에 제거될 수 있습니다.
        // 이를 처리하는 방법은 OnValidate()를 참조하세요.
        ...
        // 클라이언트 전송 속도는 현재 오류를 방지하기 위해 서버 전송 속도를 따릅니다.
        /// <summary>초당 클라이언트 업데이트 빈도입니다. Counter-Strike와 같이 빠르게 진행되는 게임에서는 대기 시간을 최소화하기 위해 약 60Hz를 사용합니다. WoW와 같은 게임에서는 계산을 최소화하기 위해 약 30Hz를 사용합니다. EVE와 같이 속도가 느린 게임에는 약 1~10Hz를 사용하세요.</summary>
        // [Tooltip("클라이언트는 초당 'sendRate' 횟수를 브로드캐스트합니다. Counter-Strike와 같이 빠르게 진행되는 게임에는 약 60Hz를 사용하여 대기 시간을 최소화합니다. WoW와 같은 게임에는 약 30Hz를 사용하여 계산을 최소화합니다. 진행이 느린 게임에는 약 1-10Hz를 사용합니다. 이브처럼요.")]
        // 공개 int clientSendRate = 30; // 33ms

        /// <summary>오프라인이 되면 자동으로 이 장면으로 전환합니다(시작 시/연결 끊김 시/종료 시).</summary>
...
#pragma warning restore 618
        ...
        /// <summary>클라이언트를 시작하고 networkAddress를 사용하여 서버에 연결합니다.</summary>
        public void StartClient()
        {
            // 설정하기 전에 점검과 단락을 수행하십시오.
            // 재시도하면 충돌 문제가 발생하지 않습니다.
        ...
        /// <summary>네트워크 "호스트"(동일한 응용 프로그램에 있는 서버와 클라이언트)를 시작합니다.</summary>
        public void StartHost()
        {
            ...
            // 장면 변경이 필요합니까? 그런 다음 장면을 변경하고 나중에 생성합니다.
            // => 호스트 클라이언트가 연결되기 전. 클라이언트 인증이 성공하면
            // 서버는 'onlineScene'을 로드하라고 지시합니다. 우리는 그렇게 할 수 없다면
            // 서버는 여전히 'offlineScene'에 있습니다. 그러니 먼저 서버에 로드하세요.
            if (IsServerOnlineSceneChangeNeeded())
            {
...
#pragma warning restore 618
        ...
        /// <summary>서버 장면과 네트워크 전체의 모든 클라이언트 장면을 변경합니다.</summary>
        // onlineScene 또는 OfflineScene이 설정된 경우 자동으로 호출되지만
        // 게임이 진행되는 동안 장면을 다시 전환하기 위해 사용자 코드에서 호출할 수 있습니다.
        // 진행 중입니다. 그러면 자동으로 클라이언트가 준비되지 않은 상태로 설정됩니다.
        // 변경이 완료되고 다시 새로운 장면에 참여할 준비가 되었습니다.
